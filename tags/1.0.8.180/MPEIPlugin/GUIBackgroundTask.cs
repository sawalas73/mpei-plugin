...
namespace MPEIPlugin
{
  internal class GUIBackgroundTask
  {
    ...
    void StopBackgroundTask(bool byUserRequest)
    {
      ...
      {
        Log.Info("[MPEI] Aborting background thread: {0}", _CurrentTaskDescription);
    ...
    internal bool ExecuteInBackgroundAndCallback(Func<object> task, Action<bool, object> resultHandler, string taskDescription, bool timeout)
    {
      ...
      if (!IsBusy && Monitor.TryEnter(this))
      {
        try
        {
          ...
          backgroundThread = new Thread(delegate()
          {
            ...
            {
              if (!abortedByUser)
                Log.Info("[MPEI] Timeout waiting for results.");
            ...
            catch (Exception threadException)
            {
              ...
              Log.Info("[MPEI] {0}", threadException.ToString());
      ...
      {
        Log.Info("[MPEI] Another thread tried to execute a task in background.");
        return false;
      }
    }
    ...
    void ExecuteTaskResultHandler()
    {
      ...
      if (_CurrentTaskSuccess != true)
      {
        if (_CurrentError != null)
        {
          ...
          GUIUtils.ShowOKDialog(Translation.Name, lines);
        }
        else
        {
          if (!abortedByUser)
          {
            if (_CurrentTaskSuccess.HasValue)
              GUIUtils.ShowNotifyDialog(Translation.Name, string.Format("{0} {1}", Translation.Error, _CurrentTaskDescription));
            else
              GUIUtils.ShowNotifyDialog(Translation.Name, string.Format("{0} {1}", Translation.Timeout, _CurrentTaskDescription));
          }
        }
      }
