...
namespace MPEIPlugin
{
    public class GUIMpeiPlugin : GUIInternalWindow, ISetupForm, IComparer<GUIListItem>, IShowPlugin
    {
        ...
        void client_DownloadProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            if (_dlgProgress != null)
            {
                _dlgProgress.SetLine(2, string.Format("{0} / {1} ({2}%)", e.TotalBytesToReceive, e.BytesReceived, e.ProgressPercentage));
                _dlgProgress.ShowProgressBar(true);
                _dlgProgress.SetPercentage(e.ProgressPercentage);
                _dlgProgress.Progress();
            }
        }
        ...
        void OnClick(int itemIndex)
        {
            ...
            //          _dlgProgress.Reset();
            //          _dlgProgress.SetHeading(14010);
            //          _dlgProgress.SetLine(1, pk.InstallerInfo.Name+" - "+pk.InstallerInfo.Version);
            //          _dlgProgress.SetLine(2, "");
            //          _dlgProgress.SetPercentage(0);
            //          _dlgProgress.Progress();
            //          _dlgProgress.DisableCancel(true);
